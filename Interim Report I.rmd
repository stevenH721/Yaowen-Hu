...
<<<<<<< HEAD
homeMortgage<-ny_hmda_2015[which(ny_hmda_2015$V8 != "" & ny_hmda_2015$V71 != "" &ny_hmda_2015V19=3),] 
omit.ny<- newdata

for(n in c(1:78)){print(sum(is.na(ny_hmda_2015[,n])))}
```{r}

```

missingdata.df <- data.frame(matrix(NA, nrow = 78, ncol = 1))
for(n in c(1:78)){
  missingdata.df[n,1] = sum(is.na(hmda[[n]]))
}
missingdata.df

ny_hmda_2015$isMissingV8<-ifelse(ny_hmda_2015$V8=="",0,1)
summary(ny_hmda_2015$V72)
...
geom_bar(stat='identity',colour="white", fill ="yellow") +
geom_text(aes(x = applicant_sex_name, y=1,label=paste0(CountOfActionTaken)),
            hjust=0, vjust=.5, size = 4, colour = 'black',
            fontface = 'bold') +
...
geom_bar(stat='identity',colour="white", fill ="yellow") +
geom_text(aes(x = applicant_ethnicity_name, y=1,label=paste0(CountOfActionTaken)),
            hjust=0, vjust=.5, size = 4, colour = 'black',
            fontface = 'bold') +
...
ggplot(aes(applicant_income_000s)) +
  ...
  labs(x = 'Income in Thousands', y = 'Count', title = 'Applicant Income distribution for loan action taken') +  theme_bw()+facet_wrap(~action_taken_name)
```
...
newdata %>% 
mutate(Loan_Approval)
if(newdata$V1==1|newdata$V1==2|newdata$V1==6)
{Loan_Approval<-1} else{Loan_Approval<-0}

Loan_Approval[newdata$V1==1|newdata$V1==2|newdata$V1==6]<-1

newdata$Loan_Approval <- ifelse(newdata$V1==1|newdata$V1==2|newdata$V1==6,1,0)
table(newdata$Loan_Approval)
...
summary(ny_hmda_2015$V20)
summary(ny_hmda_2015$V53)
model1<-glm(newdata$Loan_Approval~newdata$V72+newdata$V71+newdata$V19+factor(newdata$V9),family = binomial(link = "logit"),data = newdata)
a = newdata$Loan_Approval
model1<-glm(a~newdata$V72+newdata$V71+newdata$V19+factor(newdata$V9),family = binomial(link = "logit"),data = newdata)
